...
namespace BubbleShooterGameToolkit.Scripts.Gameplay.PlayObjects.Attachables.Covers
{
    class CoverRandom : Cover
    {
        ...
        {
            if (ball.name == "Ball Placeholder")
            {
                var ball = PoolObject.GetObject("Ball " + ColorManager.instance.GenerateColor()).GetComponent<Ball>();
                // set position on the field
                ball.SetPosition(options.DestroyedBy.position);
